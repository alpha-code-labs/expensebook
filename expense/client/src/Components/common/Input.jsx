...
const Input = ({conversionAmount, title, placeholder, onChange ,error,initialValue,type,inputRef , variant,dataMsg}) => {
  ...
  return (
    <div className={`${variant ? variant : 'w-full'} min-w-[200px]  h-[73px] flex-col justify-start items-start gap-2 inline-flex mb-3`}>
      ...
      {dataMsg?.set && (
        <div className=" absolute  top-[48px]  w-full text-xs text-yellow-200 font-cabin">
          {dataMsg?.msg}
        </div>
      )} 
