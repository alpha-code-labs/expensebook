...
const filterTrips = async (req, res) => {
  try {
    const { error, value } = tripFilterSchema.validate({ ...req.params, ...req.body });
    if (error) {
      return res.status(400).json({ message: error.details[0].message, trips: [], success: false });
    }
    ...
    const updatedTrips = extractTrip(tripDocs).map(trip => ({ ...trip, groupName: getGroups }));
    return res.status(200).json({ success: true, trips: updatedTrips });
  } catch (e) {
    console.error(e);
    res.status(500).json({ message: e.message });
...
const buildFilterCriteria = ({
  ...
  if (filterBy && date && (!fromDate && !toDate)) {
    ...
    filterCriteria.tripCompletionDate = { $gte: new Date(fromDate), $lte: new Date(toDate) };
  }
