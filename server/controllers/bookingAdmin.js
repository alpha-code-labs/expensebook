...
export const cancelTripAtHeaderLevel = async (req, res) => {
    try {
      ...
      const updateItineraryType = (itineraryType) => {
        trip.tripSchema.travelRequestData.itinerary[itineraryType].forEach((item) => {
      ...
      trip.tripSchema.travelRequestData.travelRequestStatus = trip.tripSchema.travelRequestData.travelRequestStatus === 'booked' ? 'paid and cancelled' : 'cancelled';
     ...
     if(!tripCancelled){
      ...
      await sendToOtherMicroservice(cash, 'full-update', 'expense', 'to update travelRequestStatus and cashAdvances status in expense microservice- after cancellation of entire trip')
    ...
    //   await sendToDashboardMicroservice(cash, 'full-update-cash', 'trip cancelled at header level - update "cashSchema" in dashboard', 'trip', 'batch', 'true');
      ...
      await  sendToOtherMicroservice(cash, 'full-update', 'trip', 'to update travelRequestStatus and cashAdvances status in cash microservice- after cancellation of entire trip')
    ...
    //   await sendToDashboardMicroservice(travel, 'full-update-travel', 'trip cancelled at header level - update "travelRequestSchema" in dashboard', 'trip', 'batch', 'true');
      ...
      await  sendToOtherMicroservice(travel, 'full-update', 'trip', 'to update entire travel request in Travel microservice- after cancellation of entire trip ')
      }
...
const itineraryLineItem = async (trip, recoverItinerary) => {
  ...
  export const recoveryAtLineItemLevel = async (req, res) => {
    try {
      ...
      const trip = await dashboard.findOneAndUpdate(
        {
          ...
          $or: [
            { 'tripSchema.travelRequestData.itinerary.flights.status': 'paid and cancelled' },
            { 'tripSchema.travelRequestData.itinerary.hotels.status': 'paid and cancelled' },
            { 'tripSchema.travelRequestData.itinerary.cabs.status': 'paid and cancelled' },
            { 'tripSchema.travelRequestData.itinerary.buses.status': 'paid and cancelled' },
            { 'tripSchema.travelRequestData.itinerary.trains.status': 'paid and cancelled' },
          ],
        },
        {
          $set: {
      ...
      const lineItemStatusUpdate = await itineraryLineItem(trip, recoverItinerary);
     const { isCashAdvanceTaken} = trip?.tripSchema.travelRequestData
      ...
      if(isCashAdvanceTaken){
      //send to cash microservice
       await  sendToOtherMicroservice(cash, 'full-update', 'trip', 'to update entire travel and cashAdvances data in cash microservice- recovery after cancellation of trip at itinerary level')
      ...
      //send to other microservices
      await sendToOtherMicroservice(travel, 'full-update', 'trip', 'to update entire travel request in Travel microservice- recovery after cancellation of trip at itinerary level')     
