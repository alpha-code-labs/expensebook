    ...
    try {
        ...
        if (assignTravelAdmin.modifiedCount > 0) {
            ...
            if(isCashAdvanceTaken){
                ...
                await sendToOtherMicroservice(payload,action,'cash',' to assign travel admin for ticket booking')
            } else{
                console.log("to travel assigned to")

                await sendToOtherMicroservice(payload,action,'travel', 'to assign travel admin for ticket booking' )
            }
            return res.status(200).json({ success: true, message });
        } else {
            // If no documents were modified, handle as a failure
