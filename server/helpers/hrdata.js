...
const getPolicyDetails = async (tenantId, empId, travelType) => {
    try {
      ...
      const extractedPolicies = extractPolicies(employeeGroups, Array.isArray(travelType) ? travelType : [travelType]);
      ...
      extractedPolicies.forEach(policy => {
        ...
        if (!policyByTravelType[travelType] || 
            expenseReportDeadline.dayLimit.days < policyByTravelType[travelType].expenseReportDeadline.dayLimit.days) {
