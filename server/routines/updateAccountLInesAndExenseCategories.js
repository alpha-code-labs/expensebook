...
async function updateAccountLinesAndExpenseCategories(tenantId){
    ...
    try{    
        ...
        const hrCompany = await HRCompany.findOne({tenantId}, {tenantId:1, travelCategoriesExpenseAllocation:1, policies:1, accountLines:1})
        const policies = hrCompany.policies.nonTravelPolicies
        const accountLines = hrCompany.accountLines

        const travelExpenseCategories = hrCompany.travelCategoriesExpenseAllocation.map(c=>c.categoryName).map(c=> ({categoryName:c, fields:[]}))
        ...
        if(policies.length>0){
          ...
          const expenseCategoriesNames = [...travelExpenseCategories, ...nonTravelExpenseCategories].map(category=>category.categoryName)
          ...
          //update account lines####
          await HRCompany.findOneAndUpdate({tenantId}, {$set: {accountLines:travelExpenseCategories.map(category=>({categoryName:category, accountLine:''}))}})
