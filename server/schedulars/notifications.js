...
import FB_NOTIFICATION, { notificationTypeEnums } from '../models/fbNotification.js';
...
const determineTravelStatus = async (travelDate, currentDate, tripName, travelStartDate) => {
    try {
...
const createOrUpdateBookingNotification = async ({ tenantId, messageText, status }) => {
    try {
        const existingNotification = await FinanceNotification.findOne({ tenantId });
