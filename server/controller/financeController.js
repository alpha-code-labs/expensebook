...
const getFinanceView = async (tenantId, empId) => {
  try {
    const { employeeRoles, employeeData } = await getEmployeeRoles(
      tenantId,
      empId
    );
    // console.log("employeeRoles",employeeRoles)
    const applicableRoles = Object.keys(employeeRoles).filter(
      (role) => role == "finance" && employeeRoles[role]
    );
    const finance = applicableRoles.includes("finance")
