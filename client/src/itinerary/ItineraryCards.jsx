...
export function FlightCard({item,itineraryId ,status, from, to, date, returnDate, time, travelClass, onClick, mode = 'Flight', id, handleEdit, handleDelete }) {
    return (
        <CardLayout>
            ...
            <ActionButtons id={id} item={item} itineraryId={itineraryId} status={status} handleDelete={handleDelete} handleEdit={handleEdit} />
        </CardLayout>
    )
}

export function CabCard({item, itineraryId ,status,from, to, date, returnDate, time, isFullDayCab, travelClass, mode, id, handleDelete, handleEdit }) {
...
export function RentalCabCard({item,itineraryId ,status, from, to, date, returnDate, time, travelClass, mode, id, handleDelete, handleEdit }) {
    return (
        <CardLayout>
            <div className='font-semibold items-center flex flex-col'>
                <img src={spitImageSource(mode)} className='w-6 md:h-6' />
...
export function HotelCard({item ,itineraryId ,status, checkIn, checkOut, location, ratings='any', onClick, id, handleDelete, handleEdit, needBreakfast, needLunch, needDinner, needNonSmokingRoom }) {
    return (
        <CardLayout>
...
function ActionButtons({item, status, id,itineraryId, handleDelete, handleEdit }) {
    return (
        <div className="pl-2 flex flex-row sm:flex-col sm:w-8 gap-8 sm:gap-4 cursor-pointer justify-center">
            <div onClick={() => { handleEdit(id,item) }} className="w-6 h-6 flex items-center justify-center">
                <img src={edit_icon} className="w-6 h-6 hover:w-5 hover:h-5" />
            </div>

            <div onClick={() => { handleDelete(id,status,itineraryId) }} className="w-6 h-6 flex items-center justify-center">
