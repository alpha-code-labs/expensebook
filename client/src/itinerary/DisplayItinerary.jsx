...
import {empty_itinerary_icon} from '../assets/icon';
...
export default function({formData, setFormData, handleEdit, handleDelete}) {
    ...
    useEffect(()=>{
        const array = flattenObjectToArray(formData.itinerary).filter(item=>item.id != null && item.id != undefined).sort((a,b)=> a.sequence - b.sequence)
        setFlattendedItinerary(array);
                        ...
                        if(item.category == 'flights'){
                            ...
                            <SortableItem id={item.id} key={item.id} >
                                    <FlightCard
                                        ...
                                        id={item.id} 
                                        from={item.status === "booked" ? item.bkd_from :item.from} 
                                        to={item.status === "booked" ? item.bkd_to :item.to} 
                                        date={item.status === "booked" ? item.bkd_date :item.date}
                                        returnDate={item.status === "booked" ? item.bkd_returnDate :item.returnDate}
                                        returnTime={item.status === "booked" ? item.bkd_returnTime :item.returnTime}
                                        travelClass={item.status === "booked" ? item.bkd_travelClass :item.travelClass} 
                                        mode={'Flight'}
                                        time={item.status === "booked" ? item.bkd_time :item.time}/>
                            </SortableItem>)
                        }
                        ...
                        if(item.category == 'cabs'){
                            return (<SortableItem id={item.id} key={item.id} >
                                ...
                                itineraryId={item.itineraryId} 
                                    ...
                                    handleEdit={handleEdit}
                                    id={item.id} 
                                    from={item.status === "booked" ? item.bkd_pickupAddress : item.pickupAddress} 
                                    to={item.status === "booked" ? item.bkd_dropAddress : item.dropAddress} 
                                    date={item.status === "booked" ? item.bkd_date : item.date}
                                    returnDate={item.status === "booked" ? item.bkd_returnDate : item.returnDate}
                                    isFullDayCab={item.status === "booked" ? item.bkd_isFullDayCab : item.isFullDayCab}
                                    travelClass={item.status === "booked" ? item.bkd_class : item.class} 
                                    mode={'Cab'}
                                    time={item.status === "booked" ? item.bkd_time : item.time}/>
                            </SortableItem>)
                        }
                        ...
                        if(item.category == 'hotels'){
                            return (<SortableItem id={item.id} key={item.id} >
                                ...
                                itineraryId={item.itineraryId} 
                                    ...
                                    handleEdit={handleEdit}
                                    id={item.id} 
                                    checkIn={item.status === "booked" ? item.bkd_checkIn  : item.checkIn} 
                                    checkOut={item.status === "booked" ? item.bkd_checkOut  : item.checkOut} 
                                    location={item.status === "booked" ? item.bkd_location  : item.location}
                                    time={item.status === "booked" ? item.bkd_preferredTime  : item.preferredTime}
                                    needBreakfast={item.status === "booked" ? item.bkd_needBreakfast  : item.needBreakfast}
                                    needLunch={item.status === "booked" ? item.bkd_needLunch  : item.needLunch}
                                    needDinner={item.status === "booked" ? item.bkd_needDinner  : item.needDinner}
                                    needNonSmokingRoom={item.status === "booked" ? item.bkd_needNonSmokingRoom  : item.needNonSmokingRoom}
                                    />
                            </SortableItem>)
                        }
