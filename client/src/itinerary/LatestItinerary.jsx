...
import { arrow_left_icon, close_icon ,itinerary_icon} from "../assets/icon";
import { useNavigate } from 'react-router-dom';
import Button from '../components/common/Button';
import { updateTravelRequest_API } from '../utils/api';
import Error from '../components/common/Error';

export default function({addedItineraries,bookedItineraryIds,setModifiedTripsIds,modifiedTripsIds,setBookedItineraryIds,setAddedItineraries, formData, setFormData, onBoardingData, lastPage, nextPage}){
...
console.log('bookedItineraryIds for cancel',bookedItineraryIds) //for cancel
console.log('new itineraries',addedItineraries)
...
const conirmDeleteItineraryItem = (formId,status,itineraryId)=>{
  setShowConfirm(true);
  setDeleteId({formId,status,itineraryId});
}

const editItineraryItem = useCallback((formId,itemItn)=>{
  console.log('editing item with formId', formId,itemItn); 
  let category = null;
  let item = null;
  ...
  switch(category){
    case 'flights' : {
     setModalContent(<FlightForm setVisible={setVisible} handleAddToItinerary={handleAddToItinerary} action='edit' editId={formId} itemItn={itemItn} editData={{from:item.bkd_from, to:item.bkd_to, date:item.bkd_date, time:item.bkd_time, itineraryId:item.itineraryId, status:item.status}} />);
     return; 
    }
    case 'trains' : {
      setModalContent(<TrainForm setVisible={setVisible} handleAddToItinerary={handleAddToItinerary} action='edit' editId={formId} itemItn={itemItn} editData={{from:item.bkd_from, to:item.bkd_to, date:item.bkd_date, time:item.bkd_time}} />);
      return; 
     }
     case 'buses' : {
      setModalContent(<BusForm setVisible={setVisible} handleAddToItinerary={handleAddToItinerary} action='edit' editId={formId} itemItn={itemItn} editData={{from:item.from, to:item.to, date:item.date, time:item.time}} />);
      return; 
     }
    case 'cabs' : {
      setModalContent(<CabForm setVisible={setVisible} handleAddToItinerary={handleAddToItinerary} action='edit' editId={formId} itemItn={itemItn} editData={{pickupAddress:item.pickupAddress, dropAddress:item.dropAddress, class:item.class, time:item.time, date:item.date, returnDate:item.returnDate, isFullDayCab:item.isFullDayCab}} />);
      return;
    }
    case 'carRentals' : {
      setModalContent(<CabForm setVisible={setVisible} handleAddToItinerary={handleAddToItinerary} action='edit' editId={formId} itemItn={itemItn} editData={{pickupAddress:item.pickupAddress, dropAddress:item.dropAddress, class:item.class, time:item.time, date:item.date, returnDate:item.returnDate, isRentalCab:true}} />);
      return;
    }
    case 'hotels' : {
      setModalContent(<HotelForm setVisible={setVisible} handleAddToItinerary={handleAddToItinerary} action='edit' editId={formId} itemItn={itemItn} editData={{checkIn:item.checkIn, checkOut:item.checkOut, class:item.class, location:item.location, time:item.time, needBreakfast:item.needBreakfast, needLunch: item.needLunch, needDinner:item.needDinner, needNonSmokingRoom:item.needNonSmokingRoom}} />);
      return;
    }
  }
...
const handleAddToItinerary = (category, data)=>{
  ...
  const itineraryId = data?.updatedData?.itineraryId
  const status = data?.updatedData?.status
  console.log('itineraryId1',itineraryId)
  const { id, ...updatedDataWithoutId } = data?.updatedData || {};
  console.log('modified11',updatedDataWithoutId)
    ...
    switch(category){
      case 'flights' : {
        if(data.action == 'create'){
          ...
          ///for create itinerary
            const formData_created = addedItineraries.flights.push({...updatedDataWithoutId,...newItem})
            setAddedItineraries(prev=>({...prev,formData_created}))  
        ...
        if(data.action == 'edit'){
          ...
          //for modify trip
          if(status==="booked"){
            setModifiedTripsIds(prev=>[...prev, itineraryId])
          ...
          newItem.formId = generateUniqueIdentifier();
          newItem.approvers = formData.approvers; 
          setAddedItineraries(prev => ({ ...prev, flights: [...prev.flights, { ...updatedDataWithoutId, ...newItem }] }));
          }
          //for modify trip
        }
      ...
      case 'trains' : {
        if(data.action == 'create'){
          ...
          setFormData(formData_copy);
          const formData_created = addedItineraries.trains.push({...newItem})
          setAddedItineraries(prev=>({...prev,formData_created})) 
        }
        ...
        if(data.action == 'edit'){
          ...
          if(status==="booked"){
            ...
            newItem.formId = generateUniqueIdentifier();
            newItem.approvers = formData.approvers; 
          setAddedItineraries(prev => ({ ...prev, trains: [...prev.trains, { ...updatedDataWithoutId, ...newItem }] }));
          }
        }
      ...
      case 'buses' : {
        if(data.action == 'create'){
          ...
          setFormData(formData_copy);
          const formData_created = addedItineraries.buses.push({...newItem})
          setAddedItineraries(prev=>({...prev,formData_created})) 
        }
        ...
        if(data.action == 'edit'){
          ...
          setAddedItineraries(prev => ({ ...prev, buses: [...prev.buses, { ...updatedDataWithoutId, ...newItem }] }));
          }
        }
      ...
      case 'cabs' : {
        if(data.action == 'create'){
          ...
          setFormData(formData_copy)
          const formData_created = addedItineraries.cabs.push({...newItem})
          setAddedItineraries(prev=>({...prev,formData_created})) 
        }
        ...
        if(data.action == 'edit'){
          ...
          if(status==="booked"){
            ...
            // newItem.sequence = getNextSequenceNumber();
             newItem.formId = generateUniqueIdentifier();
            newItem.approvers = formData.approvers; 
          setAddedItineraries(prev => ({ ...prev, cabs: [...prev.cabs, { ...updatedDataWithoutId, ...newItem }] }));
          }
        }
      ...
      case 'rentalCabs' : {
        if(data.action == 'create'){
          ...
          setFormData(formData_copy)
          const formData_created = addedItineraries.carRentals.push({...newItem})
          setAddedItineraries(prev=>({...prev,formData_created})) 
        }
        ...
        if(data.action == 'edit'){
          ...
          if(status==="booked"){
            ...
            newItem.formId = generateUniqueIdentifier();
            newItem.approvers = formData.approvers;  
          setAddedItineraries(prev => ({ ...prev, carRentals: [...prev.carRentals, { ...updatedDataWithoutId, ...newItem }] }));
          }
        }
      ...
      case 'hotels' : {
        if(data.action == 'create'){
          ...
          setFormData(formData_copy)
          const formData_created = addedItineraries.hotels.push({...newItem})
          setAddedItineraries(prev=>({...prev,formData_created})) 
        }
        ...
        if(data.action == 'edit'){
          ...
          if(status==="booked"){
           setModifiedTripsIds(prev=>[...prev, itineraryId])
            ...
            newItem.formId = generateUniqueIdentifier();
            newItem.approvers = formData.approvers; 
          setAddedItineraries(prev => ({ ...prev, hotels: [...prev.hotels, { ...updatedDataWithoutId, ...newItem }] }));
          }
        }
        setVisible(false);
        return;
    ...
    <>     
        <div className="max-w-[712px] mx-auto min-h-[100%] flex flex-col sm:px-8 px-6 py-6">
            ...
            <div className='flex items-center gap-4 cursor-pointer mb-4'>
                <img className='w-[24px] h-[24px]' src={arrow_left_icon} onClick={()=>navigate(lastPage)} />
            ...
            <Confirm setBookedItineraryIds={setBookedItineraryIds} visible={showConfirm} setVisible={setShowConfirm} itemId = {deleteId} actionHandler={deleteItineraryItem} />
        </div>
    </>)
}

const FlightForm = ({itemItn,setVisible, handleAddToItinerary, action='create', editId = null, editData=null})=>{

  const [formData, setFormData] = useState({from:editData?.from??'', to:editData?.to??'', date:editData?.date??getCurrentDate(), time:editData?.time??'12pm - 3pm', itineraryId:editData?.itineraryId??null , status:editData?.status??""});
  ...
  const handleSubmit = ()=>{
    ...
    if(goAhead){
      const updatedData= {...itemItn,...formData}
      handleAddToItinerary('flights', {action, editId, formData,updatedData});
      console.log('itemItn',itemItn)
...
const TrainForm = ({itemItn,setVisible, handleAddToItinerary, action='create', editId = null, editData=null})=>{
  ...
  const handleSubmit = ()=>{
    ...
    if(goAhead){
      const updatedData= {...itemItn,...formData}
      handleAddToItinerary('trains', {action, editId, formData,updatedData});
...
const BusForm = ({itemItn,setVisible, handleAddToItinerary, action='create', editId = null, editData=null})=>{
...
const CabForm = ({itemItn,setVisible, handleAddToItinerary, action='create', editId = null, editData=null})=>{
  ...
  const handleSubmit = ()=>{
    ...
    if(goAhead){
      ...
      if(formData.isRentalCab) {
        ...
        handleAddToItinerary('rentalCabs', {action, editId, formData,updatedData});
      }else handleAddToItinerary('cabs', {action, editId, formData,updatedData});
    }
...
const HotelForm = ({itemItn,setVisible, handleAddToItinerary, action='create', editId = null, editData = null})=>{
  ...
  const handleSubmit = ()=>{
    ...
    if(goAhead){
      ...
      handleAddToItinerary('hotels', {action, editId, formData,updatedData});
    }
...
const Confirm = ({setBookedItineraryIds, visible, setVisible, actionHandler, itemId}) => {
 
 console.log('id for delete',itemId?.formId)
 const formId = itemId?.formId
 const itineraryId = itemId?.itineraryId
  useEffect(()=>{
    if(visible){
      document.body.style.overflowY='hidden';
    }
  ...
  const onConfirm = ()=>{
      actionHandler(formId);
      if(itemId.status=== "booked"){
      setBookedItineraryIds((prev)=>[...prev,itineraryId])
      }
      setVisible(false);
  }
