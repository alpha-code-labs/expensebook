...
function TabItem({activeTab, setActiveTab, tab}){
    ...
    return(
        <View className={`transition-all w-[100px] whitespace-nowrap flex flex-col gap-1 ${activeTab==tab.name? 'border-indigo-600 bg-gray-100' : 'border-white hover:shadow-sm  cursor-pointer'  } border-b  border-b-2  py-1 px-2 rounded-t justify-center items-center`} onClick={()=>setActiveTab(tab.name)}>
            <Image source={spitImageSource(tab.name)} className="w-6"/>
