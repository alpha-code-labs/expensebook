...
async function updateTravelRequest_API(data){
  ...
  try{
    const {tenantId, employeeId, tripDetails, submitted,tripId} = data
    const res = await axios.post(`${TRIP_API_URL}/api/fe/trips/${tenantId}/${employeeId}/${tripId}/modify`, tripDetails , {retry, retryDelay})
...
async function getOnboardingData_API(data){
  try{
    ...
    const res = await axios.get(`${TRIP_API_URL}/api/fe/trips/${tenantId}/${employeeId}/${travelType}`)
    if(res.status >=200 && res.status<300){
...
async function getTrip_API(data){
  try{
    const {tenantId,empId,tripId} = data
    const res = await axios.get(`${TRIP_API_URL}/api/fe/trips/${tenantId}/${empId}/${tripId}/trip`)
