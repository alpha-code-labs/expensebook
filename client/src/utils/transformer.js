export const transformTripData = (tripData) => {
    const flights = tripData.travelRequestData.itinerary.flights.filter(item => item?.status !== "cancelled" && item?.status !== 'paid and cancelled') || [];
    const buses = tripData.travelRequestData.itinerary.buses.filter(item => item?.status !== "cancelled" && item?.status !== 'paid and cancelled') || [];
    const trains = tripData.travelRequestData.itinerary.trains.filter(item => item?.status !== "cancelled" && item?.status !== 'paid and cancelled') || [];
    const hotels = tripData.travelRequestData.itinerary.hotels.filter(item => item?.status !== "cancelled" && item?.status !== 'paid and cancelled') || [];
    const cabs = tripData.travelRequestData.itinerary.cabs.filter(item => item?.status !== "cancelled" && item?.status !== 'paid and cancelled') || [];
