  ...
  //get travel request Id from params
    ...
    useEffect(()=>{
        (async function(){
            ...
            setTravelRequestNumber(res?.data?.travelRequest?.travelRequestNumber??"...")
            setIsLoading(false)
        })()
    },[])
            ...
            <div className="w-full h-full mt-10 p-10 font-cabin tracking-tight">
                ...
                <div className='w-full bg-slate-50 px-6 py-4 rounded-md mt-10'>
                    ...
                    <div className="flex justify-between mt-6">
                        <div className="text-neutral-500 px-4 py-2 hover:bg-slate-200 bg-slate-100 hover:text-neutral-400 cursor-pointer" onClick={()=>navigate(`/cancel/travel/${travelRequestId}`)}>Cancel</div>
                        <div className="text-neutral-500 px-4 py-2 hover:bg-slate-200 bg-slate-100 hover:text-neutral-400 cursor-pointer" onClick={()=>navigate(`/modify/travel/${travelRequestId}`)}>Modify/Resubmit</div>
                    </div>
                </div>
            </div>
