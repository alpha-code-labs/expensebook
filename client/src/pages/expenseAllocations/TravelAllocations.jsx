...
import { getTenantOrgHeaders_API, getTenantTravelAllocations_API, postTenantTravelAllocation_API } from '../../utils/api';
...
const [loadingErr, setLoadingErr] = useState(null)
    ...
    useEffect(() => {
        (async function(){
            ...
            const allocation_res = await getTenantTravelAllocations_API({tenantId})

            const flags = flags_res.data.flags

            if(res.err || allocation_res.err){
                setLoadingErr(res.err??allocation_res)
            ...
            setSelectedOrgHeaders(allocation_res.data.travelAllocation)
            setIsLoading(false)
    ...
    const saveTravelAllocationHeaders = async () => {
        ...
        setIsLoading(true)
        const res = await postTenantTravelAllocation_API({tenantId, allocationHeaders: selectedOrgHeaders})
        setIsLoading(false)
