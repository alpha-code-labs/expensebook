...
const Settlement = ({fetchData}) => {  
 ...
 const {tenantId, empId, page} = useParams()
  ...
  let filteredData
if(cashAdvanceData){
  let filteredTravelRequests = filterTravelRequests(cashAdvanceData);
          ...
          {filteredData && filteredData.length > 0 &&  <div className=' absolute right-[-1px] top-[-8px] w-fit p-[6px] bg-green-200 border border-white-100 rounded-full '/>}
                  <div
                    className={`cursor-pointer rounded-xl  py-1 px-2 w-auto min-w-[100px] truncate${
                      activeScreen === 'Settling CashAdvance' ? 'font-medium rounded-xl bg-purple-500 text-xs text-white-100 w-auto min-w-[100px] truncate' : 'rounded-xl bg-white-100 text-neutral-800'
                    }`}
                ...
                <div
                  className={` rounded-xl cursor-pointer py-1 px-2 w-auto min-w-[100px] truncate ${
                    activeScreen === 'Recovering CashAdvance' ? 'font-medium rounded-xl bg-indigo-600 text-xs text-white-100 w-auto min-w-[100px] truncate' : 'bg-white-100 text-neutral-800'
                  }`}
...
<div className="box-border mx-4  mt-[46px]  w-auto    border-[1px]  border-b-gray "/>
         ...
         <div className='overflow-y-auto overflow-x-hidden  mt-6  w-dvh px-10'>
            ...
            {filteredData && filteredData?.map((travelDetails ,index)=>(
              ...
              <SettlingCashAdvance {...travelDetails}   employeeRole={employeeRoles?.employeeInfo?.employeeDetails} handleTravel={handleTravel}/>
              </div>
             ))}
           </div>
           </>}
   ...
   <div className="w-auto h-6 flex flex-row gap-3 ml-8 mt-7 items-center">
      <img className="w-6 h-6" src={receipt} alt="receipt" />
      <div className="text-base tracking-[0.02em] font-bold w-auto">Recovering CashAdvance</div>
    </div>
    <div className="box-border mx-4  mt-[46px]  w-auto    border-[1px]  border-b-gray "/>
    <div className='overflow-y-auto overflow-x-hidden  mt-6  w-dvh px-10'>
           {/* {tripArray.map((travelDetails, index)=> */}
           <RecoveringCashAdvance  handleTrip={handleTrip}/>
           </div>
           </>
           }
          </div>
