...
import { getTenantNonTravelExpenseAllocations_API, getTenantOrgHeaders_API, updateFormState_API, updateNonTravelAllocation_API, updateNonTravelPolicies_API } from '../../utils/api'
...
export default function (props) {
    ...
    const savePolicyAndAllocationHeaders = async()=>{
        ...
        if(allocation_res.err){
            console.log(err)
            alert('Could not update allocation headers')
            return
        }
        ...
        if(policies_res.err){
            alert('Could not update policies')
            console.log(res.err)
            return
        }
    ...
    useEffect(()=>{
        (async function(){
            ...
            const expense_allocations_res = await getTenantNonTravelExpenseAllocations_API({tenantId})

            if(res.err || expense_allocations_res.err){
                console.log(res.err??expense_allocations_res.err)
                //handle error
            }
            else{
                ...
                setSelectedOrgHeaders(expense_allocations_res.data.nonTravelExpenseAllocation)
            }
    ...
    const handleAddCategory = async ()=>{
        ...
        if(res.err){
            alert('Could not add/update category')
        }
        else{
                ...
                <div className='mt-10 flex flex-col gap-4'>  
                    {policies.map((policy,index)=>{
                            ...
                            if(ruleEngineState[0][Object.keys(ruleEngineState[0])[0]][travelType][policy]){
                                return (<Policy key={index} handleEditFields={handleEditFields} loading={loading} savePolicies={savePolicyAndAllocationHeaders} title={policy} tripType={travelType} ruleEngineState={ruleEngineState} setRuleEngineState={setRuleEngineState} selectedOrgHeaders={selectedOrgHeaders} orgHeaders={orgHeaders} handleAllocationHeaderChange={handleAllocationHeaderChange} setShowAddHeaderModal={setShowAddHeaderModal} />)
                            }
                        }
                    )}
    ...
    return (
        <div className={`w-full p-6 transition-max-height duration-1000 ${collapse? 'max-h-[75px]' : 'max-h-[2000px]'} bg-white rounded-xl border border-neutral-200 font-cabin`}>
            ...
            <div className={`w-full h-fit bg-white transition-opacity transition-max-height delay-1000  ${collapse? 'hidden opacity-0 max-height-0': 'opacity-100 max-height-[1000px]' }`} >
                ...
                <ExpenseAllocation orgHeaders={orgHeaders} setShowAddHeaderModal={setShowAddHeaderModal} selectedOrgHeaders={selectedOrgHeaders} handleAllocationHeaderChange={handleAllocationHeaderChange} title={title} />

            <div className='mt-4 flex flex-row-reverse'>
                <HollowButton title='Save Changes' isLoading={loading} onClick={()=>savePolicies()} />
            </div>
...
function ExpenseAllocation({orgHeaders, selectedOrgHeaders, setShowAddHeaderModal, handleAllocationHeaderChange, title}){
    return(<>
        {orgHeaders.length>0 && 
        <div className='mt-10'>
            ...
            <div className='flex flex-col gap-1 mt-2'>
                ...
                <div key={headerIndex} className='flex items-center justify-between'>
                    ...
                    <div className='flex-1 inline-flex'>
                        <Checkbox checked={selectedOrgHeaders.find(c=>c.categoryName == title)?.allocations?.find(h=>h.headerName == header.headerName)} onChange={(e)=>handleAllocationHeaderChange(e, title, headerIndex)} />
                    </div>
                </div>)}
            </div>
