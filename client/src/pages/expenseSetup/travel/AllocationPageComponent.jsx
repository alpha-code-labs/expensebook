...
import {postTenantTravelAllocations_API, postTravelCategories_API } from '../../../utils/api'
...
export default function ({allocations, setAllocations, tenantId, travelType, orgHeaders, setOrgHeaders, setShowTravelType}) {
    ...
    const handleContinue = async ()=>{
        const res = await updateTravelAllocations();
        //naviage to others section
        if(res.err){
            setPrompt({showPrompt:true, success: false, promptMsg: res.err??'Something went wrong please try again later'});
            return;
        }

        setPrompt({showPrompt: false, success: false, promptMsg: 'Changes saved successfully!'})
        setShowTravelType(null);
