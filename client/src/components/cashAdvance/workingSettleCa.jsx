...
import { handleCash } from '../../utils/actionHandler';



const SettleCashAdvance = ({ travelDetails, employeeRole ,handleCash}) => {
 
    console.log("got i got 14", travelDetails)
    const {actionedUpon, amountDetails, travelRequestId,cashAdvanceStatus, createdBy,paidBy,recoveredBy, cashAdvanceId,} = {...travelDetails}
...
<div>
  ...
  <div>
      <p className='font-cabin font-normal  text-xs text-neutral-400'>amountDetails</p>
       <p className='lg:text-[14px] text-[16px] text-left font-medium tracking-[0.03em] text-neutral-800 font-cabin lg:truncate '>{amountDetails.amount}</p>
       </div>
</div>
...
<div className='flex h-[52px] items-center justify-start w-[221px] py-3 px-2 '>
 ...
 {/* MARK AS SETTLEMENT */}
<div className=" flex h-[52px] items-center justify-start w-[221px] py-3 px-2 ">
  <div onClick={()=> handleCash(travelRequestId,cashAdvanceId,"ca-pay")} className="font-bold text-[14px]   truncate    lg:truncate   h-[17px] text-purple-500 text-center">
        MARK AS SETTLEMENT
  </div>
</div>      
  </div>
